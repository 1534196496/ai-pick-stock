...
### 12.1 用户确认的最终端口覆盖（2026-08-24）

用户明确要求最终 V2 Web 使用宿主机 `127.0.0.1:8502`，因为现有域名 Caddy 上游已经指向该端口。该确认覆盖“生产并行验证后仍使用 18080”的默认端口，但不改变开发阶段隔离原则：

- 开发和发布门禁期间 V2 继续使用 `127.0.0.1:18080`。
- 切换前使用旧 Compose 已支持的 `APP_PORT=8501` 将旧 Streamlit 保留在 `127.0.0.1:8501`，不修改旧镜像、代码或数据卷。
- 验证旧服务 8501 健康后，停止 V2 的 18080 绑定并以生产覆盖绑定 `127.0.0.1:8502`。
- Caddy 继续反代 `127.0.0.1:8502`；失败时停止 V2，并以 `APP_PORT=8502` 恢复旧服务。
- API 仍只绑定 `127.0.0.1:18000`，PostgreSQL 仍不映射宿主机端口。
